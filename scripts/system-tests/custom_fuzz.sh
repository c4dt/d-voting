...
# already be running. It runs every system test once, then varies their documented
...
mapfile -t TESTS < <(find "$SCRIPT_DIR" -maxdepth 1 -type f -name '*.sh' \
    ! -name 'common.sh' ! -name 'custom_fuzz.sh' ! -name 'run_all.sh' -printf '%f\n' | sort)
...
for test in "${TESTS[@]}"; do
    [[ "$test" == "load.sh" ]] || ORDERED_TESTS+=("$test")
done
for test in "${TESTS[@]}"; do
    [[ "$test" != "load.sh" ]] || ORDERED_TESTS+=("$test")
done
...
run_one() {
    ...
    [[ "$test" != "load.sh" ]] || LOAD_RUNS=$((LOAD_RUNS + 1))
